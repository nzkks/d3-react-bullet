...
class HARevenueTrackerChart extends Component {
  constructor(props) {
    super(props);
    this.init();
    this.state = {
  ...
  renderBullet() {
    ...
    let range = wrap.selectAll('rect.range').data(rangeZ);
    ...
    measure
      ...
      .attr('data-measure', function (d, i) {
        return d;
      })
      .attr('width', w1)
  ...
  renderAxis() {
    ...
    axis.attr('transform', vertical ? null : 'translate(0,' + extentY + ')').call(xAxis.scale(x1));
  }
  renderTitle() {
    if (!this.props.title) {
      return;
  ...
  getTitle() {
    ...
    if (subtitleT) {
      ...
      subtitle = (
        <text className="subtitle" dy="1em" style={subtitleStyle}>
          {subtitleT}
        </text>
      );
    } else {
    ...
    return (
      <g style={style} className={titleCls} data-title={title} data-subtitle={subtitleT}>
  ...
  render() {
    ...
    return (
      <svg width={width} height={height} className={themeCls} style={themeStyle}>
...
HARevenueTrackerChart.defaultProps = {
  vertical: true,
  reverse: false,
...
export default HARevenueTrackerChart;
